...
export default function MemberChoirHistoryPage() {
    ...
    const loadMore = useCallback(async () => {
        ...
        try {
            ...
            setPhotosError(err.message || 'Unable to load more photos.');
        } finally {
    ...
    const openLightbox = useCallback(async (photo) => {
        ...
        try {
            ...
            setLightbox((prev) => (prev ? { ...prev, loading: false, error: 'Unable to load this photo.' } : prev));
        }
    }, []);
    ...
    return (
        <div className="space-y-8 pb-14">
            ...
            <header className="relative overflow-hidden rounded-3xl bg-blue-700 p-7 text-white shadow-xl shadow-blue-900/10 sm:p-10">
                ...
                <div className="relative max-w-3xl">
                    <div className="mb-4 flex items-center gap-3 text-blue-100">
                        <BookOpen size={20} />
                        <span className="text-xs font-black uppercase tracking-[0.2em]">Our story</span>
                    </div>
                    ...
                    <p className="mt-3 max-w-2xl text-sm leading-6 text-blue-100">
                        A private archive of the people, moments, worship, and milestones that shaped your choir.
                    </p>
                </div>
            </header>
            ...
            <section className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_360px]">
                <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
                    <div className="mb-5 flex items-center justify-between gap-4">
                        <div>
                            <p className="text-xs font-black uppercase tracking-[0.16em] text-blue-600">The story</p>
                            <h2 className="mt-1 text-2xl font-black text-slate-900">Where we come from</h2>
                        </div>
                        {canManage && (
                            <button
                                type="button"
                            ...
                            >
                                <Check size={15} /> {t('common.submit', 'Save story')}
                            </button>
                        )}
                    </div>
                    {canManage ? (
                        <textarea
                            ...
                            placeholder="Write the story of your choir..."
                        />
                    ) : (
                        <p className="whitespace-pre-wrap text-sm leading-8 text-slate-600">
                            {history || 'Your choir history has not been written yet.'}
                        </p>
                    )}
                </div>
                    ...
                    <div className="mt-5 space-y-5">
                        <div className="flex gap-3">
                            ...
                            <div>
                                ...
                                <p className="font-black text-slate-900">{formatDate(data?.choir?.founded_at)}</p>
                            </div>
                        </div>
                        <div className="flex gap-3">
                            <Camera className="shrink-0 text-blue-600" size={20} />
                            <div>
                                <p className="text-xs font-bold text-slate-500">Historical photos</p>
                                <p className="font-black text-slate-900">{photoMeta.total} archived</p>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
                <div className="mb-8">
                    <p className="text-xs font-black uppercase tracking-[0.16em] text-blue-600">Milestones</p>
                    <h2 className="mt-1 text-2xl font-black text-slate-900">Moments that shaped us</h2>
                </div>
                <div className="relative ml-2 space-y-7 border-l-2 border-blue-100 pl-7">
                    {(data?.milestones || []).map((item) => (
                        <div key={item.id} className="relative">
                            ...
                            <p className="text-xs font-black uppercase tracking-wider text-blue-600">{formatDate(item.date)}</p>
                            <h3 className="mt-1 text-lg font-black text-slate-900">{item.title}</h3>
                            <p className="mt-1 text-sm leading-6 text-slate-500">{item.description}</p>
                        </div>
                    ))}
                    {!(data?.milestones || []).length && (
                        <EmptyState icon={Clock3} title="No milestones yet" message="Important dates will appear as the choir's story grows." />
                    )}
                </div>
            </section>
            ...
            <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
                <div className="mb-8">
                    <p className="text-xs font-black uppercase tracking-[0.16em] text-blue-600">Archive</p>
                    <h2 className="mt-1 text-2xl font-black text-slate-900">Historical activities</h2>
                </div>
                <div className="grid gap-4 md:grid-cols-2">
                    ...
                    {!(data?.events || []).length && (
                        <EmptyState icon={CalendarDays} title="No historical activities yet" message="Past performances and rehearsals will appear here." />
                    )}
                </div>
            </section>
            ...
            <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
                <div className="mb-6 flex flex-wrap items-end justify-between gap-4">
                    <div>
                        <p className="text-xs font-black uppercase tracking-[0.16em] text-blue-600">Memory wall</p>
                        <h2 className="mt-1 text-2xl font-black text-slate-900">Historical photographs</h2>
                    </div>
                    {canManage && (
                        <button
                            type="button"
                ...
                {!photosLoading && !photos.length ? (
                    <EmptyState icon={ImagePlus} title="No historical photos yet" message="The choir's photo archive will appear here when the first memory is saved." />
                ) : (
                    <>
                                    ...
                                    <article key={photo.id} className="group overflow-hidden rounded-2xl border border-slate-200 bg-white">
                                        ...
                                        >
                                            {failed ? (
                                                ...
                                                <img
                                                    src={url}
                                                    alt={photo.title || 'Historical choir memory'}
                                                    loading="lazy"
                                                    decoding="async"
                                        ...
                                        <div className="p-4">
                                            <h3 className="font-black text-slate-900">{photo.title || 'Choir memory'}</h3>
                                            <p className="mt-1 text-xs text-slate-400">{formatDate(photo.event_date)}</p>
                                            {canManage && (
                        ...
                        {photosLoadingMore && (
                            <div className="mt-6 flex items-center justify-center gap-2 py-4 text-xs font-semibold text-slate-400">
                                <Loader2 size={16} className="animate-spin" /> Loading more photos...
                            </div>
                        )}
